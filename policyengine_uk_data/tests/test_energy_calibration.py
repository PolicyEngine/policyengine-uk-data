...
def arrays(imputed):
    ...
    return dict(
        income  = sim.calculate("hbai_household_net_income", map_to="household", period=2023).values,
        tenure  = sim.calculate("tenure_type",               map_to="household", period=2023).values,
        accomm  = sim.calculate("accommodation_type",        map_to="household", period=2023).values,
        region  = sim.calculate("region",                    map_to="household", period=2023).values,
        weights = sim.calculate("household_weight",          map_to="household", period=2023).values,
...
def test_electricity_by_income(arrays):
    ...
    _print_table("Electricity £/yr by income band", [(b, i, t, e) for b, i, t, e, _ in rows])
    for band, imp, target, pct_err, tol in rows:
        assert pct_err < tol, f"Electricity by income [{band}]: {imp:.0f} vs {target:.0f} ({pct_err:.1%})"
...
def test_gas_by_income(arrays):
    ...
    for band, imp, target, pct_err, tol in rows:
        assert pct_err < tol, f"Gas by income [{band}]: {imp:.0f} vs {target:.0f} ({pct_err:.1%})"

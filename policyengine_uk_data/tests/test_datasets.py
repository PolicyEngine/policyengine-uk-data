...
def test_calibration_preserves_totals_roughly(frs):
    ...
    relative_change = (
        abs(new_total_weight - original_total_weight) / original_total_weight
    )
    assert (
        relative_change < 0.5
    )  # Less than 50% change in total (calibration can change totals significantly)
    ...
    def estimate_function(w, matrix_arg=None):
        ...
        if isinstance(w, torch.Tensor):
            ...
            matrix_torch = torch.tensor(
                matrix.T, dtype=torch.float32, device=device
            )
...
def test_enhanced_frs_has_more_data(frs, enhanced_frs):
    ...
    assert len(enhanced_frs.household.household_id) == len(
        frs.household.household_id
    )

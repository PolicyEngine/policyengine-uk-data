...
def create_firm(year: int = 2023):
    ...
    pe_firm["vat_registered"] = synthetic_df["vat_registered"].astype(bool).values
    ...
    pe_firm["employment_band"] = pe_firm["employment"].apply(_map_employment_to_band)
    ...
    dataset.metadata = {
        ...
        "vat_registered_firms": pe_firm[pe_firm["vat_registered"]]["firm_weight"].sum(),
        "total_employment": (pe_firm["employment"] * pe_firm["firm_weight"]).sum(),
        "total_turnover_billions": (pe_firm["annual_turnover_k"] * pe_firm["firm_weight"]).sum() / 1e6,
        "total_vat_liability_billions": (pe_firm["vat_liability_k"] * pe_firm["firm_weight"]).sum() / 1e6,
    }
    
    logger.info(f"Created firm dataset with {len(pe_firm):,} firms")
    logger.info(f"Total weighted population: {dataset.metadata['total_weighted_firms']:,.0f}")
    logger.info(f"Total employment: {dataset.metadata['total_employment']:,.0f}")
    logger.info(f"Total turnover: £{dataset.metadata['total_turnover_billions']:.1f}bn")
...
class firm_2023_24:
    ...
    def __init__(self):
        ...
        if dataset_path.exists():
            ...
            self.business_group = pd.read_hdf(dataset_path, key='business_group')
        else:
...
if __name__ == "__main__":
    ...
    for key, value in firm_dataset.metadata.items():
        if isinstance(value, (int, float)):
            ...
            elif key in ["n_firms", "total_weighted_firms", "vat_registered_firms", "total_employment"]:

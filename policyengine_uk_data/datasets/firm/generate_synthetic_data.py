...
try:
    ...
    # If can't import, use a default storage location
...
class SyntheticFirmGenerator:
    ...
    def __init__(self, device: str = "cpu", random_seed: int = 42):
    ...
    def load_data(
        ...
        ons_path = project_root / "ONS_UK_business_data" / "firm_turnover.csv"
        ons_employment_path = (
            project_root / "ONS_UK_business_data" / "firm_employment.csv"
        )
        hmrc_turnover_path = (
            project_root
            / "HMRC_VAT_annual_statistics"
            / "vat_population_by_turnover_band.csv"
        )
        hmrc_sector_path = (
            project_root
            / "HMRC_VAT_annual_statistics"
            / "vat_population_by_sector.csv"
        )
        vat_liability_sector_path = (
            project_root
            / "HMRC_VAT_annual_statistics"
            / "vat_liability_by_sector.csv"
        )
        vat_liability_band_path = (
            project_root
            / "HMRC_VAT_annual_statistics"
            / "vat_liability_by_turnover_band.csv"
        )
        ...
        logger.info(
            f"Loaded ONS employment data: {len(ons_employment_df)} rows"
        )
        ...
        logger.info(
            f"Loaded VAT liability by sector data: {len(vat_liability_sector_df)} rows"
        )
        logger.info(
            f"Loaded VAT liability by band data: {len(vat_liability_band_df)} rows"
        )

        # Extract ONS total
        ons_total_row = ons_df[
            ons_df["SIC Code"].isna() | (ons_df["SIC Code"] == "")
        ]
        if len(ons_total_row) > 0:
            ons_total = ons_total_row.iloc[0]["Total"]
        else:
            sector_rows = ons_df[
                ~ons_df["Description"].str.contains("Total", na=False)
            ]
        ...
        return (
            ...
            vat_liability_band_df,
            ons_total,
        )

    def generate_base_firms(
        self, ons_df: pd.DataFrame
    ) -> Tuple[Tensor, Tensor]:
        ...
        """
        logger.info(
            "Generating base firms using efficient batch processing..."
        )
        ...
        sic_codes_tensor = torch.tensor(
            [int(sic) for sic in all_sic_codes],
            dtype=torch.int64,
            device=self.device,
        )
        turnover_tensor = torch.tensor(
            all_turnovers, dtype=torch.float32, device=self.device
        )
    ...
    def _generate_turnover_values(
        self,
        band_name: str,
        firm_count: int,
        min_turnover: float,
        max_turnover: float,
        midpoint_turnover: float,
    ) -> Tensor:
        ...
        noise_std = max(
            25.0, band_width * 0.2
        )  # Uniform noise: 25k minimum or 20% of band width
    ...
    def assign_vat_registration_flags(
        self, turnover_values: Tensor, hmrc_bands: Dict[str, int]
    ) -> Tensor:
        ...
        hmrc_target_1_to_threshold = hmrc_bands[
            "£1_to_Threshold"
        ]  # HMRC target: 678,350
        synthetic_1_to_threshold = (
            ((turnover_values > 0) & (turnover_values <= 85.0)).sum().item()
        )  # Current synthetic count
        voluntary_rate = (
            hmrc_target_1_to_threshold / synthetic_1_to_threshold
            if synthetic_1_to_threshold > 0
            else 0.15
        )

        logger.info(
            f"Calculated voluntary VAT rate: {voluntary_rate:.3f} (Target: {hmrc_target_1_to_threshold:,} / Synthetic: {synthetic_1_to_threshold:,})"
        )
        ...
        if n_below_threshold > 0:
            ...
            voluntary_mask = (
                torch.rand(len(turnover_values), device=self.device)
                < voluntary_rate
            )
        ...
        logger.info(
            f"VAT registration: {mandatory_vat.sum():.0f} mandatory + {voluntary_vat.sum():.0f} voluntary = {vat_registered.sum():.0f} total"
        )
        logger.info(f"Non-VAT registered: {(~vat_registered).sum():.0f} firms")

        return vat_registered

    def create_comprehensive_target_matrix(
        self,
        turnover_values: Tensor,
        sic_codes: Tensor,
        input_values: Tensor,
        hmrc_bands: Dict[str, int],
        hmrc_sector_df: pd.DataFrame,
        ons_employment_df: pd.DataFrame,
        vat_liability_sector_df: pd.DataFrame,
        vat_liability_band_df: pd.DataFrame,
        ons_total: int,
    ) -> Tuple[Tensor, Tensor]:
        ...
        sector_rows = hmrc_sector_df[
            hmrc_sector_df["Trade_Sector"] != "Total"
        ].copy()
        hmrc_total = hmrc_sector_df[hmrc_sector_df["Trade_Sector"] == "Total"][
            "2023-24"
        ].iloc[0]
        ...
        vat_liability_bands = {
            ...
            "Greater_than_£10m": vat_liability_band_latest[
                "Greater_than_£10m"
            ],
        }
        ...
        n_targets = (
            7 + n_sectors + n_employment_bands + n_vat_sectors + n_vat_bands
        )  # 7 turnover + sector + employment + VAT liability by sector + VAT liability by band
        ...
        employment_band_indices = torch.tensor(
            [
                map_employment_to_band_idx(emp.item())
                for emp in employment_values
            ],
            dtype=torch.long,
            device=self.device,
        )
        ...
        for i, (_, vat_row) in enumerate(vat_liability_sector_rows.iterrows()):
            ...
            target_matrix[row_idx, firms_in_sector] = vat_liability_values[
                firms_in_sector
            ]
        ...
        for i, band_name in enumerate(
            ...
            if band_name == "£1_to_Threshold":
                ...
                firms_in_band = (turnover_values > 85) & (
                    turnover_values <= 150
                )
            elif band_name == "£150k_to_£300k":
                firms_in_band = (turnover_values > 150) & (
                    turnover_values <= 300
                )
            elif band_name == "£300k_to_£500k":
                firms_in_band = (turnover_values > 300) & (
                    turnover_values <= 500
                )
            elif band_name == "£500k_to_£1m":
                firms_in_band = (turnover_values > 500) & (
                    turnover_values <= 1000
                )
            elif band_name == "£1m_to_£10m":
                firms_in_band = (turnover_values > 1000) & (
                    turnover_values <= 10000
                )
            else:  # Greater_than_£10m
                firms_in_band = turnover_values > 10000

            # Weight by VAT liability for this band's target
            target_matrix[row_idx, firms_in_band] = vat_liability_values[
                firms_in_band
            ]
        ...
        for band in emp_bands:
            if band in ons_employment_df.columns:
                sector_rows_emp = ons_employment_df[
                    ~ons_employment_df["Description"].str.contains(
                        "Total", na=False
                    )
                ]
        ...
        for band_name in [
            ...
            vat_liability_millions = vat_liability_bands[
                band_name
            ]  # in millions £
            vat_liability_k = vat_liability_millions * 1000  # convert to £k
        ...
        target_values = torch.tensor(
            target_values_list, dtype=torch.float32, device=self.device
        )
        ...
        logger.info(
            f"  Sector targets: {n_sectors} sectors from HMRC data (VAT-registered)"
        )
        logger.info(
            f"  Employment count targets: {n_employment_bands} bands from ONS data (direct counts)"
        )
        logger.info(
            f"  VAT liability sector targets: {n_vat_sectors} sectors from HMRC data (in £millions)"
        )
        logger.info(
            f"  VAT liability band targets: {n_vat_bands} bands from HMRC data (in £millions, 2x weight)"
        )
        logger.info(
            f"  Total targets: {len(target_values_list)} (7 turnover + {n_sectors} sector + {n_employment_bands} employment + {n_vat_sectors} VAT liability sector + {n_vat_bands} VAT liability band)"
        )
    ...
    def optimize_weights(
        ...
        log_weights = torch.zeros(
            n_firms, device=self.device, requires_grad=True
        )
        ...
        for iteration in range(n_iterations):
            ...
            n_remaining = (
                n_total_targets - 7 - n_employment
            )  # Remove turnover and employment
            n_est_sectors = (
                n_remaining // 3
            )  # Rough estimate (sectors, vat_sectors, vat_bands)
            ...
            importance_weights[sector_start_idx:emp_start_idx] = (
                1.0  # 1x weight for sector targets
            )
            importance_weights[emp_start_idx:vat_sector_start_idx] = (
                1.0  # 1x weight for employment targets
            )
            importance_weights[vat_sector_start_idx:vat_band_start_idx] = (
                1.0  # 1x weight for VAT liability sector targets
            )
            importance_weights[vat_band_start_idx:] = (
                2.0  # 2x weight for VAT liability band targets
            )
            ...
            if iteration % 100 == 0:
                logger.info(
                    f"Iteration {iteration}: Loss = {total_loss.item():.6f}"
                )
        ...
        for i, (pred, target, name) in enumerate(
            zip(final_predictions, target_values, target_names)
        ):
            if target > 0:  # Only log targets we're actually trying to match
                accuracy = 1 - abs(pred - target) / target
                logger.info(
                    f"  {name}: {pred:.0f} vs {target:.0f} ({accuracy:.1%})"
                )
            else:
    ...
    def apply_final_calibration(
        self,
        base_sic_codes: Tensor,
        base_turnover: Tensor,
        base_input: Tensor,
        ...
        if negative_zero_target > 0:
            ...
            unique_sics, counts = torch.unique(
                base_sic_codes, return_counts=True
            )
            total_current_firms = len(base_sic_codes)

            logger.info(
                f"Manually adding {negative_zero_target:,} zero turnover firms from HMRC..."
            )
            zero_firms_count = 0

            for sic, count in zip(unique_sics, counts):
                sector_weight = count.float() / total_current_firms
                firms_for_sector = int(
                    (negative_zero_target * sector_weight).item()
                )
        ...
        if all_final_firms:
            final_sics = torch.tensor(
                [f["sic_code"] for f in all_final_firms],
                dtype=torch.int64,
                device=self.device,
            )
            final_turnover = torch.tensor(
                [f["annual_turnover_k"] for f in all_final_firms],
                dtype=torch.float32,
                device=self.device,
            )
            final_input = torch.tensor(
                [f["annual_input_k"] for f in all_final_firms],
                dtype=torch.float32,
                device=self.device,
            )
            final_weights = torch.tensor(
                [f["weight"] for f in all_final_firms],
                dtype=torch.float32,
                device=self.device,
            )
    ...
    def _map_to_hmrc_bands(self, turnover_values: Tensor) -> Tensor:
        ...
        band_indices = torch.where(
            turnover_values <= 0, 0, band_indices
        )  # Negative_or_Zero
        band_indices = torch.where(
            (turnover_values > 0) & (turnover_values <= 85), 1, band_indices
        )  # £1_to_Threshold
        band_indices = torch.where(
            (turnover_values > 85) & (turnover_values <= 150), 2, band_indices
        )  # £Threshold_to_£150k
        band_indices = torch.where(
    ...
    def _print_accuracy_breakdown(
        self, accuracies: list, n_items: int, label: str
    ):
        ...
        print(
            f"Accuracy breakdown: ≥95%: {accuracy_95_plus}/{n_items}, "
    ...
    def generate_input_values(
        self, turnover_values: Tensor, sic_codes: Tensor
    ) -> Tensor:
        ...
        logger.info(
            f"  Firms with negative VAT liability (input > output): {(final_ratios > 1.0).sum().item():,}"
        )

        return input_values

    def assign_employment(
        self, num_firms: int, ons_employment_df: pd.DataFrame
    ) -> Tensor:
        ...
        for band in emp_bands:
            target_count = int(
                round(num_firms * total_ons_counts[band] / total_ons_firms)
            )
            if target_count > 0:
                ...
                if band == "0-4":
                    ...
                    values = torch.randint(
                        1, 5, (target_count,), device=self.device
                    )
                elif band == "250+":
                    # Log-normal for large firms
                    log_mean = torch.log(
                        torch.tensor(midpoint, device=self.device)
                    )
                    values = torch.normal(
                        log_mean, 0.8, (target_count,), device=self.device
                    ).exp()
                    ...
                    beta_values = uniform.pow(0.5) * (1 - uniform).pow(
                        2.0
                    )  # Beta approximation
        ...
        if len(employment_values) < num_firms:
            employment_values.extend(
                [1] * (num_firms - len(employment_values))
            )
        elif len(employment_values) > num_firms:
            employment_values = employment_values[:num_firms]

        employment_array = torch.tensor(
            employment_values, dtype=torch.float32, device=self.device
        )
    ...
    def validate_comprehensive_accuracy(
        ...
        hmrc_target_bands: Dict[str, int],
        ons_total_target: int,
        ons_employment_df: pd.DataFrame,
        hmrc_sector_df: pd.DataFrame,
        vat_liability_df: pd.DataFrame,
        vat_liability_band_df: pd.DataFrame,
        ...
        """
        logger.info(
            "Validating synthetic data against official data sources..."
        )
        ...
        synthetic_df["hmrc_band"] = synthetic_df["annual_turnover_k"].apply(
            map_to_hmrc_band
        )
        all_bands = (
            synthetic_df.groupby("hmrc_band")["weight"]
            .sum()
            .round()
            .astype(int)
        )
        ...
        vat_registered_count = synthetic_df[
            synthetic_df["vat_registered"] == True
        ]["weight"].sum()
        hmrc_total_vat = sum(hmrc_target_bands.values())
        vat_accuracy = (
            1 - abs(vat_registered_count - hmrc_total_vat) / hmrc_total_vat
        )
        ...
        print(
            f"Difference:                {vat_registered_count - hmrc_total_vat:+,.0f}"
        )
        ...
        print(
            f"{'Band':>25} {'Synthetic':>12} {'Target':>12} {'Source':>8} {'Accuracy':>10}"
        )
        print("-" * 75)
        ...
        for band_name, target_count in hmrc_target_bands.items():
            ...
            accuracy = (
                1 - abs(synthetic_count - target_count) / target_count
                if target_count > 0
                else (1.0 if synthetic_count == 0 else 0.0)
            )
            ...
            if band_name == "£1_to_Threshold":
                ...
                status = (
                    "✓" if accuracy > 0.90 else "⚠" if accuracy > 0.80 else "✗"
                )
                source = "HMRC"

            print(
                f"  {status} {band_name:>22}: {synthetic_count:>10,} vs {target_count:>10,} {source:>8} ({accuracy:>6.1%})"
            )

        hmrc_accuracy = (
            np.mean(hmrc_band_accuracies) if hmrc_band_accuracies else 0.0
        )
        print("-" * 75)
        ...
        ons_population_accuracy = (
            1
            - abs(total_synthetic_weighted - ons_total_target)
            / ons_total_target
        )
        ...
        print(
            f"Difference:      {total_synthetic_weighted - ons_total_target:+,.0f}"
        )
        ...
        for band in employment_bands:
            if band in ons_employment_df.columns:
                ...
                ons_employment_targets[band] = (
                    sector_rows[band].fillna(0).sum()
                )
            else:
                ons_employment_targets[band] = 0

        # Calculate synthetic employment distribution
        synthetic_df["employment_band"] = synthetic_df["employment"].apply(
            _map_employment_to_band
        )
        synthetic_employment_counts = (
            synthetic_df.groupby("employment_band")["weight"]
            .sum()
            .round()
            .astype(int)
        )
        ...
        print(
            f"{'Band':>8} {'Synthetic':>10} {'ONS Target':>11} {'Accuracy':>10}"
        )
        print("-" * 65)
        ...
        for band in employment_bands:
            ...
            accuracy = (
                1 - abs(synthetic_count - ons_target) / ons_target
                if ons_target > 0
                else (1.0 if synthetic_count == 0 else 0.0)
            )
            ...
            print(
                f"  {status} {band:>6}: {synthetic_count:>8,} vs {ons_target:>9,} ({accuracy:>6.1%})"
            )

        print("-" * 65)
        employment_accuracy = self._print_accuracy_breakdown(
            employment_accuracies, len(employment_bands), "EMPLOYMENT"
        )
        ...
        vat_registered_firms = synthetic_df[
            synthetic_df["vat_registered"] == True
        ]
        synthetic_vat_sector_counts = (
            vat_registered_firms.groupby("sic_numeric")["weight"]
            .sum()
            .round()
            .astype(int)
        )

        self._print_validation_section(
            "SECTOR VALIDATION (VAT-registered firms)"
        )
        print(
            f"{'SIC':>5} {'Synthetic VAT':>12} {'HMRC Target':>12} {'Accuracy':>10}"
        )
        print("-" * 65)
        ...
        for _, sector_row in sector_rows.iterrows():
            ...
            accuracy = (
                1 - abs(synthetic_vat_count - hmrc_target) / hmrc_target
                if hmrc_target > 0
                else (1.0 if synthetic_vat_count == 0 else 0.0)
            )
            ...
            print(
                f"  {status} {sic_code:>3}: {synthetic_vat_count:>10,} vs {hmrc_target:>10,} ({accuracy:>6.1%})"
            )

        print("-" * 65)
        sector_accuracy = self._print_accuracy_breakdown(
            sector_accuracies, len(sector_accuracies), "SECTOR"
        )

        # === VAT LIABILITY VALIDATION ===
        # Get VAT liability sector data (excluding total)
        vat_liability_rows = vat_liability_df[
            vat_liability_df["Trade_Sector"] != "Total"
        ].copy()

        # Calculate synthetic VAT liability by sector
        synthetic_df["vat_liability_k"] = (
            synthetic_df["annual_turnover_k"] - synthetic_df["annual_input_k"]
        )
        synthetic_df["sic_numeric"] = synthetic_df["sic_code"].astype(int)

        # Group by sector and calculate weighted VAT liability
        synthetic_vat_liability = (
            synthetic_df.groupby("sic_numeric")
            .apply(lambda x: (x["vat_liability_k"] * x["weight"]).sum())
            .reset_index(name="synthetic_vat_liability_k")
        )

        self._print_validation_section("VAT LIABILITY VALIDATION (by sector)")
        print(
            f"{'SIC':>5} {'Synthetic (£m)':>14} {'Target (£m)':>12} {'Accuracy':>10}"
        )
        print("-" * 65)
        ...
        for _, vat_row in vat_liability_rows.iterrows():
            ...
            sector_data = synthetic_vat_liability[
                synthetic_vat_liability["sic_numeric"] == sic_code
            ]
            ...
            synthetic_millions = (
                synthetic_k / 1000
            )  # convert back to millions for display
            ...
            if abs(target_millions) > 0.1:  # Skip near-zero targets
                ...
                if target_millions < 0 and synthetic_millions < 0:
                    ...
                    accuracy = 1 - min(
                        abs(synthetic_millions - target_millions)
                        / abs(target_millions),
                        1.0,
                    )
            ...
            status = (
                "✓" if accuracy > 0.70 else "⚠" if accuracy > 0.50 else "✗"
            )
            print(
                f"  {status} {sic_code:>3}: {synthetic_millions:>12.1f} vs {target_millions:>10.1f} ({accuracy:>6.1%})"
            )

        print("-" * 65)
        vat_liability_sector_accuracy = self._print_accuracy_breakdown(
            vat_liability_accuracies,
            len(vat_liability_accuracies),
            "VAT LIABILITY BY SECTOR",
        )
        ...
        synthetic_vat_band_liability = (
            vat_registered_firms.groupby("hmrc_band")
            .apply(lambda x: (x["vat_liability_k"] * x["weight"]).sum())
            .reset_index(name="synthetic_vat_liability_k")
        )
        ...
        print(
            f"{'Band':>25} {'Synthetic (£m)':>14} {'Target (£m)':>12} {'Accuracy':>10}"
        )
        print("-" * 75)
        ...
        for band_name, target_millions in vat_liability_band_targets.items():
            ...
            band_data = synthetic_vat_band_liability[
                synthetic_vat_band_liability["hmrc_band"] == band_name
            ]
            ...
            print(
                f"  {status} {band_name:>22}: {synthetic_millions:>12.1f} vs {target_millions:>10.1f} ({accuracy:>6.1%})"
            )

        print("-" * 75)
        vat_liability_band_accuracy = self._print_accuracy_breakdown(
            vat_liability_band_accuracies,
            len(vat_liability_band_accuracies),
    ...
    def generate_synthetic_firms(self) -> pd.DataFrame:
        ...
        logger.info(
            f"  ONS total firms: {ons_total:,} (includes all businesses)"
        )
        logger.info(
            f"  HMRC VAT firms: {sum(hmrc_bands.values()):,} (VAT-registered only)"
        )
        ...
        final_sic_codes, final_turnover, final_input, final_weights = (
            self.apply_final_calibration(
                base_sic_codes,
                base_turnover,
                base_input,
                optimized_weights,
        ...
        employment_values = self.assign_employment(
            len(final_sic_codes), ons_employment_df
        )

        # Assign VAT registration flags
        vat_flags = self.assign_vat_registration_flags(
            final_turnover, hmrc_bands
        )
        ...
        logger.info(
            f"  Weighted population: {synthetic_df['weight'].sum():,.0f}"
        )

        # Validation against all data sources
        self.validate_comprehensive_accuracy(
            synthetic_df,
            hmrc_bands,
            ons_total,
            ons_employment_df,
...
def main():
    ...
    logger.info(
        "Generating synthetic firm records using PyTorch for efficient processing"
    )

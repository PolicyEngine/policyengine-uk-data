...
def add_benefits(person, benunit, household, _frs_person, frs: FRS, policy_parameters):
    ...
    benunit["income_support"] = sum_person_to_benunit(get_benefit_with_code(19, person, frs.benefits), person, benunit) * 52
    ...
    person["care_hours"] = np.where(get_benefit_with_code(13, person, frs.benefits) > 0, 35 + 5, 0) # If receiving carer's allowance assume 40 hours of care (35 is minimum)
    person["esa_contrib"] = (
        sum_to_entity(
            frs.benefits.BENAMT
...
def sum_person_to_benunit(values, person, benunit):
    return values.groupby(person.person_id.values).sum().reindex(benunit.benunit_id).fillna(0).values

def get_benefit_with_code(code: int, person: pd.DataFrame, benefits: pd.DataFrame):
    return pd.Series(sum_to_entity(
